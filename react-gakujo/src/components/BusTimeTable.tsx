...
function BusTimeTable() {
    ...
    const BusTimeTableAcordion = ({ list }: BusTimeTableProps) => {
        const splitList: Bus[][] = [];
        let hour = -1;
        let tmpList: Bus[] = [] as Bus[]; // hour list
        for (let i = 0; i < list.length; i++) {
            const newHour = list[i].departureTime.hour;
            console.log(newHour, hour, tmpList.length, splitList.length);
            if (hour != newHour) {
                console.log("true");
                hour = newHour;
                if (tmpList.length != 0) {
